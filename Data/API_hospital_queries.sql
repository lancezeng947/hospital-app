...
#My Community
SELECT DISTINCT sub.name AS disease_name, sub.first_name,sub.last_name,sub.email,sub.zip
FROM (
    SELECT DISTINCT p.first_name, p.last_name,p.email, p.zip,v.disease_id, d.name,SUBSTRING(zip,1,1) AS sub_zip
    FROM patients p
